...
While the sampler generated from AnalogBase is more flexible, associated
APIs need to be set up (which is not for the cds_ff_mpt technology), so
laygo based sampler is used for this document.
...
can be easily generated by placing and routing subcells. Run following
scripts to generate the SubADC top.

    ```
    # layout
    run laygo/generators/adc_sar/sar_wsamp_layout_generator.py
    # schematic
    run laygo/generators/adc_sar/sar_wsamp_schematic_generator.py
    ```
