...
if verify_tran == True:
    ...
    measstr = 'delay(?wf1 VT("/SAOM"), ?value1 VAR("vdd")/2, ?edge1 "either", ?nth1 1, ?td1 0.0, ?tol1 nil, ?wf2 VT("/ZP<%d>"), ?value2 VAR("vdd")/2, ?edge2 "either", ?nth2 1, ?tol2 nil,  ?td2 nil , ?stop nil, ?multiple nil)' % (
                num_bits - 1)
    # print(measstr)
