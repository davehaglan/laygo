...
for m, m_buf, m_rgnn, m_rst in zip(m_list, m_buf_list, m_rgnn_list, m_rst_list):
    ...
    cint1 = (mrgnp['cdd'] + mrgnp['cgg']) * m_rgnp + (mrgnn['cdd'] + mrgnn['cgg']) * m_rgnn + (
                mbufn['cgg'] + mbufp['cgg']) * m_buf + mrstn['cdd'] * m_rstn
...
print('salatch size results - m:' + str(m_opt) + ' m_rgnn:' + str(m_rgnn_opt) + ' m_rst:' + str(
    m_rst_opt) + ' m_buf:' + str(m_buf_opt))
